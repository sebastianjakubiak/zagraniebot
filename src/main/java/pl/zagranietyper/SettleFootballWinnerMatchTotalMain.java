...
import pl.zagranietyper.parser.FootballWinnerMatchTotalParser;
...
public final class SettleFootballWinnerMatchTotalMain {
     ...
     *
     * Ten rekord ma pozostać PENDING.
     *
     * Jeżeli podczas APPLY pojawi się jakikolwiek inny
     * subject issue, zatrzymujemy cały apply.
     */
    private static final long KNOWN_BAD_RESOLUTION_LEG_ID =
            12225L;

    private static final long KNOWN_BAD_RESOLUTION_FIXTURE_ID =
            1383801L;

    private static final String KNOWN_BAD_RESOLUTION_SUBJECT =
            "psg";
    ...
    private SettleFootballWinnerMatchTotalMain() {
    }
    ...
    public static void main(
        ...
        FootballWinnerMatchTotalParser parser =
        ...
        Map<FootballWinnerMatchTotalParser.Status, Integer>
                parserStatuses =
                new EnumMap<>(
                        FootballWinnerMatchTotalParser.Status.class
                );

        for (
                FootballWinnerMatchTotalParser.Status status :
                FootballWinnerMatchTotalParser.Status.values()
        ) {
            parserStatuses.put(
                    status,
                    0
        ...
        for (
            ...
            if (
                ...
                if (
                        parser.looksLikeWinnerMatchTotal(
                                candidate.tipTitle()
                        )
                ) {
                    Rejected row =
    ...
    private static boolean parseApplyFlag(
        ...
        throw new IllegalArgumentException(
                "Usage: SettleFootballWinnerMatchTotalMain [--apply]"
        );
    }
    ...
    private static boolean isSubjectIssue(
        ...
        return status
                == FootballWinnerMatchTotalParser.Status.SUBJECT_NOT_FOUND
                || status
                == FootballWinnerMatchTotalParser.Status.SUBJECT_MISMATCH
                || status
                == FootballWinnerMatchTotalParser.Status.SUBJECT_AMBIGUOUS;
    }

    private static void validateKnownSubjectIssue(
            List<Rejected> subjectIssues
    ) {
        if (
                subjectIssues.size() != 1
        ) {
            throw new IllegalStateException(
                    "REFUSING APPLY: expected exactly one known "
                            + "subject issue, actual="
                            + subjectIssues.size()
            );
        }
        ...
        boolean expected =
                candidate.legId()
                        ...
                        && candidate.fixtureId()
                        == KNOWN_BAD_RESOLUTION_FIXTURE_ID
                        && result.status()
                        == FootballWinnerMatchTotalParser.Status.SUBJECT_MISMATCH
                        && KNOWN_BAD_RESOLUTION_SUBJECT.equals(
                        result.subject()
                );
    ...
    private static void printHeader(
        ...
        System.out.println(
                "Zagranie Typer — Football WINNER + MATCH TOTAL Settlement"
        );
    ...
    private static void printParsed(
        ...
        for (
            ...
            FootballWinnerMatchTotalParser.ParseResult p =
                    row.parseResult();
    ...
    private static void printRejected(
        ...
        System.out.println(
                "WINNER+MATCH-TOTAL-LIKE REJECTED="
                        + rejected.size()
        );
    ...
    private static void printDryRunFooter() {
        ...
        System.out.println(
                "  pl.zagranietyper.SettleFootballWinnerMatchTotalMain --apply"
        );
    }
    ...
    private record Parsed(
            FootballSettlementRepository.Candidate candidate,
            FootballWinnerMatchTotalParser.ParseResult parseResult,
            SettlementDecision decision
    ) {
    }
